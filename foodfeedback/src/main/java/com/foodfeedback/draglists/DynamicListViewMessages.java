...
import com.foodfeedback.messages.MessageCoachListAdapter;
 ...
 */
public class DynamicListViewMessages extends ListView {
	...
	public DynamicListViewMessages(Context context) {
		super(context);
		init(context);
	}

	public DynamicListViewMessages(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(context);
	}

	public DynamicListViewMessages(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	...
	private void updateNeighborViewsForID(long itemID) {
		...
		MessageCoachListAdapter adapter = ((MessageCoachListAdapter) getAdapter());
	...
	private void touchEventsEnded() {
		try {
			CoachOrderingCacheManager.saveObject(this.ctx, mMemberList);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (mCellIsMobile || mIsWaitingForScrollFinish) {
			...
			// If the auto scroller has not completed scrolling, we need to wait
			// for it to finish in order to determine the final location of where 
			// the hover cell should be animated to.
