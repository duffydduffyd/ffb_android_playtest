...
package com.foodfeedback.draglists;
...
import com.foodfeedback.coaches.CoachListViewAdapter;
 ...
 */
public class DynamicListViewCoaches extends ListView {
	...
	private ArrayList<Coach> mMemberList;
	...
	public DynamicListViewCoaches(Context context) {
		super(context);
		init(context);
	}

	public DynamicListViewCoaches(Context context, AttributeSet attrs,
			int defStyle) {
...
//		this.setOverscrollFooter(getResources().getDrawable(R.drawable.hlv_overscroll_edge));
//		this.setOverscrollHeader(getResources().getDrawable(R.drawable.hlv_overscroll_edge));
	}
	...
	private void updateNeighborViewsForID(long itemID) {
		...
		CoachListViewAdapter adapter = ((CoachListViewAdapter) getAdapter());
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		for (int i = 0; i < mMemberList.size(); i++) {
			members.add(mMemberList.get(i).getMemberInfo());
		}
		try {
			CoachOrderingCacheManager.saveObject(this.ctx, members);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		case MotionEvent.ACTION_MOVE:
			System.out.println(" move event registered");
			if (mActivePointerId == INVALID_POINTER_ID) {
				break;
			}
			System.out.println(" move event registered 2 " + mCellIsMobile);
			...
			if (mCellIsMobile) {
				System.out.println(" move event registered 3");
		...
		case MotionEvent.ACTION_UP:
			System.out.println(" up event registered");
			touchEventsEnded();
			break;
		case MotionEvent.ACTION_CANCEL:
			System.out.println(" cancel event registered");
			touchEventsCancelled();
			break;
		case MotionEvent.ACTION_POINTER_UP:
			System.out.println(" pointer up event registered");
			/*
			...
			break;
		default:
			System.out.println(" degault event registered");
			break;
		}
	...
	private void handleCellSwitch() {
		...
		if (isBelow || isAbove) {
			...
			swapElements(mMemberList, originalItem,
	...
	private void touchEventsEnded() {
		...
		if (mCellIsMobile || mIsWaitingForScrollFinish) {
			System.out.println("Setting mCellIsMobile to False now");
			mCellIsMobile = false;
	...
	public void setMemberList(ArrayList<Coach> memberList) {
